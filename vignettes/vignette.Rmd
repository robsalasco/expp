...
1. Download R version R 3.0.2, for example from <a href="http://cran.rstudio.com/">cran.studio.com</a>.
...
5. The epp-function only works on individual years. Since we have two years of data, we need to split out data sets up ...  
  ```{r}
...
7. We can then paste the two years of data together.
  ```{r}
  dat = data.frame(rbind(d[[1]]@EPP, d[[2]]@EPP))
  #dat = as.data.frame(d) #combine the data sets for the individual years
  ```
  
8. We now have two data sets. "d" lists the output of the function epp() for the two years, including the input data. "dat" contains the combined data of the two seasons excluding the input data.
  
9. We can then plot the data of each year. The package supplies two different types of plots for class "epp" (output of function epp()  ).  
...
10. Since EPP is most frequent among direct neighbours, estimates for all other variable are most meaningful if they are assessed for direct neighbours. We therefore subtract '1' from the breeding distance, so that direct neighbours get scored as '0'. This transformation is important if interactions are modeled.
  ```{r}
  if(min(dat$rank == 1)) dat$rank = dat$rank - 1
  ```

11. As a next step, variables can be transformed to be relative to the surroundings of the males or the females. We here present breeding asynchrony as an example.  
  ```{r}
...
12. We can now make sure the sample size is sufficient for the number of variables we aim to include into the model.
  ```{r}
  table(dat$epp, dat$year_) #sample size as the number of "0" and "1" in the epp column
  ```
  
13. And finally, we can run the model (this may take a while depending on your system!).
  ```{r}
  require(lme4)
...
14. Finally, you can plot the model output using for example the R package "effects".
  ```{r}
  require(effects)
  plot(allEffects(fm))
  ```
