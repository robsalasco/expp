...
westerholzBreeding = exppBreeding[exppBreeding$year_ %in% c(2009, 2010), ]
westerholzBreeding$study_area = NULL 
...
```{r, echo=FALSE, results='asis'}
data(exppEPP)
westerholzEPP = exppEPP[exppEPP$year_ %in% c(2009, 2010), ]
...
breedingDat = lapply(b, SpatialPointsBreeding, coords= ~x+y, id='id', breeding= ~male + female, 
  proj4string = CRS("+proj=tmerc +lat_0=0 +lon_0=12 +k=1 +x_0=4500000 +y_0=0 +datum=potsdam +units=m +no_defs"))
...
```{r}
data(exppBoundary)
westerholzBoundary = exppBoundary[ c("2009", "2010") ]

polygonsDat = mapply(DirichletPolygons, x = breedingDat, boundary = westerholzBoundary) 
```  
********************************************************************************
